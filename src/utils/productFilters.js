export function filterProducts(products, options = {}) {
  ...
  return products.filter((product) => {
    const searchable =
      `${product.name} ${product.collection || ""} ${product.category || ""} ${product.anime || ""} ${product.label || ""} ${product.japanese || ""}`.toLowerCase();
    if (q && !searchable.includes(q)) return false;
    if (
      animeQ &&
      (!product.anime || !product.anime.toLowerCase().includes(animeQ))
    )
      return false;
